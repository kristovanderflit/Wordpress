...
class WP_To_Buffer_Pro_Publish {
    ...
    public function transition_post_status( $new_status, $old_status, $post ) {
        ...
        // Add the error to the log so that the user can see why no statuses were sent to Buffer
        WP_To_Buffer_Pro_Log::get_instance()->update_log( $post->ID, array(
            array(
                'date'              => strtotime( 'now' ),
                'success'           => false,
    ...
    public function publish( $post_id, $action, $is_bulk_publish_action = false ) {
        ...
        if ( ! $access_token ) {
            return new WP_Error( 'no_access_token', sprintf( __( 'The Plugin has not been authorized with Buffer! Go to %s > Settings to setup the plugin.', $this->base->plugin->name ), $this->base->plugin->displayName ) );
        }
